...
export function DashboardOverview() {
  ...
  const pipelineStates = [
    { key: 'PENDING', label: 'Pending', count: metrics.pendingCount, color: 'bg-slate-500' },
    { key: 'ASSIGNED', label: 'Assigned', count: metrics.assignedCount, color: 'bg-indigo-500' },
    { key: 'IN_PROGRESS', label: 'In Progress', count: metrics.inProgressCount, color: 'bg-cyan-500' },
    { key: 'SUBMITTED', label: 'Submitted', count: metrics.submittedCount, color: 'bg-blue-500' },
    { key: 'UNDER_REVIEW', label: 'Under Review', count: metrics.underReviewCount, color: 'bg-purple-500' },
    { key: 'APPROVED', label: 'Approved', count: metrics.approvedCount, color: 'bg-emerald-500' },
    { key: 'REJECTED', label: 'Rejected', count: metrics.rejectedCount, color: 'bg-rose-500' },
    { key: 'REWORK_REQUIRED', label: 'Rework Req.', count: metrics.reworkRequiredCount, color: 'bg-amber-500' },
    { key: 'COMPLETED', label: 'Completed', count: metrics.completedCount, color: 'bg-teal-500' },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-slate-900 via-indigo-950/40 to-slate-900 p-6 rounded-xl border border-slate-800/80">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-slate-100 tracking-tight">AURORA AnnotOps Control Center</h1>
            <span className="text-xs px-2 py-0.5 rounded bg-cyan-950 text-cyan-300 border border-cyan-700/60 font-semibold">
              Live Operations
            </span>
          </div>
          <p className="text-xs text-slate-400 mt-1">
            Real-time task pipeline performance, quality metrics, and SLA throughput diagnostics.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-xl bg-slate-900/80 border border-slate-800 flex flex-col justify-between hover:border-slate-700 transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-400">Total Active Tasks</span>
        ...
        <div className="p-5 rounded-xl bg-slate-900/80 border border-slate-800 flex flex-col justify-between hover:border-slate-700 transition-all">
          ...
          <div className="mt-4">
            ...
            <p className="text-xs text-slate-400 mt-1">Target: 95.0% Benchmark</p>
          </div>
        </div>
        ...
        <div className="p-5 rounded-xl bg-slate-900/80 border border-slate-800 flex flex-col justify-between hover:border-slate-700 transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-400">Avg. Review Time (MTTR)</span>
          ...
          <div className="mt-4">
            ...
            <p className="text-xs text-slate-400 mt-1">MTTA Speed: {metrics.avgAnnotationTimeMinutes}m</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 rounded-xl bg-slate-900/80 border border-slate-800 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-base font-bold text-slate-100">Task Workflow Pipeline</h2>
            <p className="text-xs text-slate-400">Current task distribution across the 9 lifecycle states</p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-9 gap-3">
          {pipelineStates.map((st) => (
            <div key={st.key} className="p-3 rounded-lg bg-slate-950 border border-slate-800 flex flex-col justify-between">
              <div className="flex items-center gap-1.5 mb-2">
                <span className={`w-2 h-2 rounded-full ${st.color}`} />
                <span className="text-[11px] font-medium text-slate-300 truncate">{st.label}</span>
              </div>
              <span className="text-xl font-bold text-slate-100">{st.count.toLocaleString()}</span>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="p-6 rounded-xl bg-slate-900/80 border border-slate-800 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400">Daily annotated vs. reviewed volumes</p>
            </div>
          </div>
          <div className="h-64 w-full pt-4">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={productivityData}>
                ...
                />
                <Bar dataKey="annotated" fill="#06B6D4" radius={[4, 4, 0, 0]} name="Annotated" />
                <Bar dataKey="reviewed" fill="#6366F1" radius={[4, 4, 0, 0]} name="Reviewed" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="p-6 rounded-xl bg-slate-900/80 border border-slate-800 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400">First-pass accuracy progression (%)</p>
            </div>
          </div>
          <div className="h-64 w-full pt-4">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={qualityTrendData}>
                ...
                <RechartsTooltip
                  contentStyle={{ backgroundColor: '#0F172A', borderColor: '#334155', borderRadius: '8px' }}
                />
                <Line type="monotone" dataKey="passRate" stroke="#10B981" strokeWidth={2.5} name="Pass Rate %" />
                <Line type="monotone" dataKey="rejectionRate" stroke="#F43F5E" strokeWidth={2} name="Rejection %" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="p-6 rounded-xl bg-slate-900/80 border border-slate-800 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400">Urgent priority items and active rework queues</p>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {urgentAttentionTasks.map((t: TaskItem) => (
                <tr key={t.id} className="hover:bg-slate-800/40 transition-colors">
                  <td className="p-3 font-mono font-semibold text-cyan-400">{t.id}</td>
                  <td className="p-3">
                    <p className="font-semibold text-slate-200">{t.title}</p>
                    <p className="text-[10px] text-slate-400">{t.datasetName}</p>
                  </td>
                  <td className="p-3">
                  ...
                  <td className="p-3 text-slate-300">{t.assigneeName || 'Unassigned'}</td>
